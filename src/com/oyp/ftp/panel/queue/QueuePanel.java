...
public class QueuePanel extends JPanel implements ActionListener {
		...
		if (orientation.equals("上传")) { // 如果是上传任务
			...
			for (int i = 0; i < size; i++) { // 遍历上传队列
				...
				if (file.getAbsolutePath().equals(value)) {
					ListSelectionModel selModel = queueTable.getSelectionModel(); // 获取表格的选择模型
			...
			for (int i = 0; i < size; i++) { // 遍历下载队列
				...
				if (file.getAbsolutePath().equals(value)) {//
					ListSelectionModel selModel = queueTable.getSelectionModel(); // 获取任务队列表格的选择模型
					// 设置模型使用单选模式
	...
	private synchronized void refreshQueue() {
		...
		if (frame.getShutdownButton().isSelected() && localQueue.isEmpty() && ftpQueue.isEmpty()) {
			try {
				int confirmDialog = JOptionPane.showConfirmDialog(frame, "确认60s后自动关机？");
				if (confirmDialog == JOptionPane.YES_OPTION) {
					Runtime.getRuntime().exec("shutdown -s -t 60");// 执行系统关机命令，延迟60秒钟
				}
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		for (int i = 0; i < localQueueArray.length; i++) {
			...
			model.addRow(new Object[] { localFile.getAbsoluteFile(), "上传", MainStatic.ftpClient.getPassiveHost(),
					i == 0 ? "正在上传" : "等待上传" });
		}
		...
		for (int i = 0; i < ftpQueueArray.length; i++) {
			...
			if (queueValue == null)
				continue;
			FTPFile ftpFiles = (FTPFile) queueValue[0];
			// 把下载队列的任务添加到表格组件的数据模型中
			model.addRow(new Object[] { ftpFiles.getName(), "下载", "服务器", i == 0 ? "正在下载" : "等待下载" });
		}
